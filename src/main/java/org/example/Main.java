...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        order.clear();
        ArrayList<Integer> finalVector = new ArrayList<>();
        Arrays.fill(used, false);
        for (int i = 0; i < minVert.size(); i++) {
            int vert = minVert.get(i);
            if (!used[vert]) {
                finalVector.add(vert);
                DFS(vert);
            }
        }
        ...
        try {
            ...
            String text1 = String.valueOf(finalVector.size());
            fileWriter.write(text1+'\n');
            for (int i = 0; i < finalVector.size(); i++) {
                String text = String.valueOf(finalVector.get(i)+1);
